...
__global__ void minimum_distance(float * X, float * Y, volatile float * D, int n) {
    ...
    if(index < n-1) {
        ...
        __syncthreads();
        int lgst_index = (n % block_size);
        if(lgst_index == 0) {
            lgst_index = block_size;
        }
        else {
        ...
        for(i = 1; i < lgst_index; i *= 2) {
            if(threadIdx.x % (2 * i) == 0 && (threadIdx.x + i) < lgst_index - 1) {
        ...
        if(threadIdx.x == 0) {
            ...
            int val = atomicInc(&finished_blocks, gridDim.x);
            isLastDone = (val == (gridDim.x - 1));
        }
        if(isLastDone && threadIdx.x == 0) {
